...
class WC_Shipping_USPS extends WC_Shipping_Method {
	...
	private $domestic        = array( 'US', 'PR', 'VI', 'MH', 'FM', 'GU' );
	private $found_rates;
			...
			if ( $package_requests ) {
				...
				if ( $response ) {
					...
					if ( ! ( $xml = $this->get_parsed_xml( $response ) ) ) {
						$this->debug( 'Failed loading XML', 'notice' );
					}

					if ( ! is_object( $xml ) && ! is_a( $xml, 'SimpleXMLElement' ) ) {
						$this->debug( 'Invalid XML response format', 'notice' );
					}
					...
					if ( ! empty( $usps_packages ) ) {
						...
						// No rates
						$this->debug( 'Invalid request; no rates returned', 'notice' );
					}
				}
			}
			...
			if ( $this->found_rates ) {
				foreach ( $this->found_rates as $key => $value ) {
					if ( $value['packages'] < sizeof( $package_requests ) ) {
						$this->debug( "Unsetting {$key} - too few packages.", 'notice' );
		...
		if ( $this->found_rates ) {
			...
			if ( isset( $this->found_rates['usps:D_PRIORITY_MAIL'] ) && isset( $this->found_rates['usps:flat_rate_box_priority'] ) ) {
				if ( $this->found_rates['usps:flat_rate_box_priority']['cost'] < $this->found_rates['usps:D_PRIORITY_MAIL']['cost']  ) {
					$this->debug( "Unsetting PRIORITY MAIL api rate - flat rate box is cheaper.", 'notice' );
					unset( $this->found_rates['usps:D_PRIORITY_MAIL'] );
				} else {
					$this->debug( "Unsetting PRIORITY MAIL flat rate - api rate is cheaper.", 'notice' );
			...
			if ( isset( $this->found_rates['usps:D_EXPRESS_MAIL'] ) && isset( $this->found_rates['usps:flat_rate_box_express'] ) ) {
				if ( $this->found_rates['usps:flat_rate_box_express']['cost'] < $this->found_rates['usps:D_EXPRESS_MAIL']['cost']  ) {
					$this->debug( "Unsetting PRIORITY MAIL EXPRESS api rate - flat rate box is cheaper.", 'notice' );
					unset( $this->found_rates['usps:D_EXPRESS_MAIL'] );
				} else {
					$this->debug( "Unsetting PRIORITY MAIL EXPRESS flat rate - api rate is cheaper.", 'notice' );
	...
	private function per_item_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $domestic ) {
				...
				$request .= '	<CommercialFlag>' . ( $this->shippingrates === 'ONLINE' ? 'Y' : 'N' ) . '</CommercialFlag>' . "\n";
				$request .= '</Package>' . "\n";
	...
	private function weight_based_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( ! $values['data']->get_weight() ) {
				$this->debug( sprintf( __( 'Product #%d is missing weight. Using 1lb.', 'woocommerce-shipping-usps' ), $item_id ), 'notice' );
	...
	private function box_shipping( $package ) {
		...
		foreach ( $package['contents'] as $item_id => $values ) {
			...
			if ( $values['data']->length && $values['data']->height && $values['data']->width ) {
				...
				$this->debug( sprintf( __( 'Product #%d is missing dimensions! Using 1x1x1.', 'woocommerce-shipping-usps' ), $item_id ), 'notice' );
		...
		if ( $flat_packages ) {
			foreach ( $flat_packages as $flat_package ) {
				...
				if ( isset( $this->flat_rate_boxes[ $flat_package->id ] ) ) {
					...
					// Get pricing
					$box_pricing  = $this->shippingrates === 'ONLINE' && isset( $this->flat_rate_pricing[ $flat_package->id ]['online'] ) ? $this->flat_rate_pricing[ $flat_package->id ]['online'] : $this->flat_rate_pricing[ $flat_package->id ]['retail'];
