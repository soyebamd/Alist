		...
		<form action="admin.php?page=wpdevart-reservations" method="post" id="reservations_form">
			<div id="action-buttons" class="div-for-clear">
				<div id="reservation_header" class="div-for-clear">
					...
					<a id="view_calendar" href="" onclick="wpdevart_set_value('task','display_month_reservations'); wpdevart_form_submit(event, 'reservations_form')"><span class="reservation-item-info"><?php _e('Reservation Month View','booking-calendar'); ?></span></a>
						...
						<div id="wpdevart_tab_2_container" class="wpdevart_container div-for-clear">
						    <div class="filter_item status_filter_item">
								...
								<div class="filter_fild_item">
									<input type="checkbox" name="reserv_status[]" id="res_approved" value="approved" <?php checked(isset($_POST["reserv_status"]) && in_array("approved",$_POST["reserv_status"])); ?>><label for="res_approved"><?php _e('Approved','booking-calendar'); ?></label>
								</div>	
								<div class="filter_fild_item">
									<input type="checkbox" name="reserv_status[]" id="res_canceled" value="canceled" <?php checked(isset($_POST["reserv_status"]) && in_array("canceled",$_POST["reserv_status"])); ?>><label for="res_canceled"><?php _e('Canceled','booking-calendar'); ?></label>
								</div>
								<div class="filter_fild_item">
									<input type="checkbox" name="reserv_status[]" id="res_rejected" value="rejected" <?php checked(isset($_POST["reserv_status"]) && in_array("rejected",$_POST["reserv_status"])); ?>><label for="res_rejected"><?php _e('Rejected','booking-calendar'); ?></label>
								</div>	
								<div class="filter_fild_item">
									<input type="checkbox" name="reserv_status[]" id="res_pending" value="pending" <?php checked(isset($_POST["reserv_status"]) && in_array("pending",$_POST["reserv_status"])); ?>><label for="res_pending"><?php _e('Pending','booking-calendar'); ?></label>
								</div>	
							</div>
			...
			<table class="wp-list-table widefat fixed pages wpdevart-table wpdevart-reservations-table"> 
				<tr>
					<thead>
						...
						<th><?php _e('Reservation information','booking-calendar'); ?></th>
						<th class="medium-column"><?php _e('Reservation dates','booking-calendar'); ?></th>
						<th class="medium-column"><?php _e('Actions','booking-calendar'); ?></th>
					</thead>
				<tr>
				<?php
				if(isset($_SESSION["clendar_id"]) && $_SESSION["clendar_id"] != 0) {
						...
						<tr>
							...
							<div class="reserv-info div-for-clear">
								<div class='reserv-info-container'>
									<h5  class="reserv-info-open-title"><?php _e('Details','booking-calendar'); ?><span class="reserv-info-open"></span></h5>
							...
							<?php if($row->status == "pending" || $row->status == "canceled" || $row->status == "rejected") { ?>
								<a href="" onclick="wpdevart_set_value('task','approve'); wpdevart_set_value('cur_id','<?php echo $row->id; ?>'); wpdevart_form_submit(event, 'reservations_form')" ><?php _e('Approve','booking-calendar'); ?></a>
								<?php if($row->status == "pending") { ?>
									<a href="" onclick="wpdevart_set_value('task','reject'); wpdevart_set_value('cur_id','<?php echo $row->id; ?>'); wpdevart_form_submit(event, 'reservations_form')" ><?php _e('Reject','booking-calendar'); ?></a>
								<?php  } ?>
							<?php } elseif($row->status == "approved") { ?>
								<a href="" onclick="wpdevart_set_value('task','canceled'); wpdevart_set_value('cur_id','<?php echo $row->id; ?>'); wpdevart_form_submit(event, 'reservations_form')" ><?php _e('Cancel','booking-calendar'); ?></a>
							<?php  } ?>
							<a href="" onclick="wpdevart_set_value('task','delete'); wpdevart_set_value('cur_id','<?php echo $row->id; ?>'); wpdevart_form_submit(event, 'reservations_form')" ><?php _e('Delete','booking-calendar'); ?></a></td>
						<tr>
				<?php	}
				 } ?>
			</table>
